...
public class Main12844 {
    ...
    public static void update(int start, int end, int idx, int left, int right, int value) {
        ...
        if(left <= start && end <= right) {
            boolean u = (end - start + 1) % 2 == 0;
            // 구간에 있을 땐 부모 노드의 값만 갱신하고 lazy에 값 담아둠, 다음번에 다른 노드가 방문할 때 한번에 갱신
            segTree[idx] = u ? segTree[idx] : segTree[idx] ^ value;
            if(start != end) {
                lazy[idx * 2] ^= value;
                lazy[idx * 2 + 1] ^= value;
            }
    ...
    public static int find(int start, int end, int idx, int left, int right) {
        ...
        int mid = (start + end) / 2;
        int l = find(start, mid, idx * 2, left, right);
        int r = find(mid + 1, end, idx * 2 + 1, left, right);
        return l ^ r;
    }
    public static void updateLazy(int start, int end, int idx) {
        if(lazy[idx] != 0) {
            boolean u = (end - start + 1) % 2 == 0;
            segTree[idx] = u ? segTree[idx] : segTree[idx] ^ lazy[idx];
            if(start != end) {
                lazy[idx * 2] ^= lazy[idx];
